...
const BACKUP_GENERATIONS_KEY='kyk_backup_generations_v01', BACKUP_MAX_GENERATIONS=5;
...
function openApproval(id){approvingId=id;const d=records().find(x=>x.id===id);if(!d)return;$('#approvalSummary').innerHTML=`<strong>${esc(d.company)}</strong><p>${esc(d.date)}／${esc(d.site)}</p><p>${esc(d.work.filter(x=>x.content).map(x=>x.content).join('、'))}</p><p>職長から：${esc(d.foremanComment||'')}</p>`;$('#principalComment').value=d.approval?.comment||d.principalCommentDraft||'';principalSign=d.approval?.signImage||'';principalPad.load(principalSign);$('#principalCount').textContent=$('#principalComment').value.length;go('approve-detail')}
...
function printableJournalHtml(list){const pages=list.map(j=>{const rows=activeKyFor(j.site,j.date).map((d,i)=>`<tr><td>${i+1}</td><td>${esc(d.company)}</td><td>${(d.work||[]).reduce((s,w)=>s+(Number(w.actual)||0),0)}</td><td>${esc(d.work?.[0]?.content||'')}</td></tr>`).join('');const lines=(title,arr)=>`<h3>${title}</h3><ol>${Array.from({length:6},(_,i)=>`<li>${esc((arr||[])[i]||'')}</li>`).join('')}</ol>`;return`<section class="pdf-page"><h1>工事日誌</h1><p><b>現場名：</b>${esc(j.site)}　<b>作業日：</b>${esc(j.date)}</p><table><thead><tr><th>No.</th><th>会社名</th><th>実施人員</th><th>作業内容1</th></tr></thead><tbody>${rows}</tbody></table>${lines('元請連絡',j.contact)}${lines('品質に関する事項',j.quality)}</section>`}).join('');return`<!doctype html><html lang="ja"><head><meta charset="utf-8"><title>日誌PDF</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:sans-serif;margin:0}.pdf-page{break-after:page;page-break-after:always}.pdf-page:last-child{break-after:auto;page-break-after:auto}h1{text-align:center;margin:0 0 8px}table{border-collapse:collapse;width:100%;font-size:12px}th,td{border:1px solid #333;padding:4px}h3{margin:10px 0 4px}ol{margin:0;padding-left:28px}li{min-height:20px;border-bottom:1px solid #aaa;padding:2px}</style></head><body>${pages}<script>window.onload=()=>setTimeout(()=>window.print(),250)<\/script></body></html>`}
function exportJournalPdfList(list){if(!list.length){toast('PDF出力対象がありません');return}const w=window.open('','_blank');if(!w){toast('ポップアップを許可してください');return}w.document.open();w.document.write(printableJournalHtml(list));w.document.close();toast(`${list.length}件をPDF印刷画面に表示しました`)}
function exportJournalPdf(id){const j=journals().find(x=>x.id===id&&!x.deleted);if(j)exportJournalPdfList([j])}
function readBackupGenerations(){try{return JSON.parse(localStorage.getItem(BACKUP_GENERATIONS_KEY)||'[]')}catch{return[]}}
function writeBackupGenerations(v){localStorage.setItem(BACKUP_GENERATIONS_KEY,JSON.stringify(v.slice(0,BACKUP_MAX_GENERATIONS)))}
...
async function createManualBackup(){try{const payload=backupPayload(),text=JSON.stringify(payload),gens=readBackupGenerations();gens.unshift({id:uid(),createdAt:payload.createdAt,appVersion:APP_VERSION,countRecords:payload.records.length,countJournals:payload.journals.length,payload:text});writeBackupGenerations(gens);const zip=new JSZip();zip.file('backup.json',text);zip.file('README.txt','KYK管理システム バックアップ\nKYKDB.xlsx本体は含まれません。');const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE'}),a=document.createElement('a'),d=payload.createdAt.replace(/[-:T]/g,'').slice(0,12);a.href=URL.createObjectURL(blob);a.download=`KYK管理バックアップ_${d}.zip`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1500);renderBackupGenerations();toast('バックアップを作成しました')}catch(e){console.error(e);toast('バックアップ作成に失敗しました：'+e.message)}}
async function restoreBackupFile(file){try{const zip=await JSZip.loadAsync(file),f=zip.file('backup.json');if(!f)throw new Error('backup.jsonがありません');const data=JSON.parse(await f.async('string'));if(data.format!=='KYK_BACKUP_V1'||!Array.isArray(data.records)||!Array.isArray(data.journals))throw new Error('対応していないバックアップです');if(!confirm(`KYK ${data.records.length}件、日誌 ${data.journals.length}件で現在のデータを置き換えます。よろしいですか？`))return;saveRecords(data.records);saveJournals(data.journals);toast('復元しました');renderDashboard();renderBackupGenerations()}catch(e){console.error(e);toast('復元に失敗しました：'+e.message)}}
function restoreGeneration(id){const g=readBackupGenerations().find(x=>x.id===id);if(!g)return;const data=JSON.parse(g.payload);if(confirm(`${new Date(g.createdAt).toLocaleString('ja-JP')}の世代へ復元しますか？`)){saveRecords(data.records||[]);saveJournals(data.journals||[]);toast('保存世代から復元しました');renderDashboard()}}
function renderBackupGenerations(){const el=$('#backupGenerationList');if(!el)return;const gs=readBackupGenerations();el.innerHTML=gs.length?`<h4>端末内バックアップ（直近5世代）</h4>${gs.map(g=>`<div class="backup-generation"><span>${new Date(g.createdAt).toLocaleString('ja-JP')}　KY ${g.countRecords}件／日誌 ${g.countJournals}件</span><button onclick="restoreGeneration('${g.id}')">復元</button></div>`).join('')}`:'<p class="meta">端末内バックアップはまだありません。</p>'}
...
window.exportJournalPdf=exportJournalPdf;window.restoreGeneration=restoreGeneration;
