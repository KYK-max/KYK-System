...
function hydrateMasters(){['#site','#searchSite','#approvalSite','#journalSearchSite'].forEach(x=>selectOptions($(x),master.sites,x==='#site'?'選択してください':'すべて'));['#company','#searchCompany'].forEach(x=>selectOptions($(x),master.companies,x==='#company'?'選択してください':'すべて'));$$('.health-select').forEach(x=>selectOptions(x,master.health));$$('.qual-select').forEach(x=>selectOptions(x,master.qualifications));$$('.poss-select').forEach(x=>selectOptions(x,master.possibilities,'選択'));$$('.sev-select').forEach(x=>selectOptions(x,master.severities,'選択'));for(let i=1;i<=8;i++)updateQualificationDisplay(i)}
...
function buildRows(){
 ...
 for(let i=1;i<=8;i++)$('#qual'+i).onchange=e=>{updateQualificationDisplay(i);e.target.blur()};
}
...
function clearWorkerSlot(i){workerSigns[i-1]='';workerPads[i-1]?.load('');if($('#health'+i))$('#health'+i).value='';if($('#qual'+i))$('#qual'+i).value=''}
...
function openJournal(id){const j=journals().find(x=>x.id===id);if(!j)return;editingJournalId=id;renderJournalEditSummary(j);for(let i=1;i<=6;i++){const c=(j.contact||[])[i-1]||'',q=(j.quality||[])[i-1]||'';$('#journalContact'+i).value=c;$('#journalContactCount'+i).textContent=c.length;$('#journalQuality'+i).value=q;$('#journalQualityCount'+i).textContent=q.length}const lastFilled=a=>Math.max(1,...a.map((v,i)=>String(v||'').trim()?i+1:0));setJournalLineVisibility('journalContact',lastFilled(j.contact||[]));setJournalLineVisibility('journalQuality',lastFilled(j.quality||[]));go('journal-edit');setTimeout(()=>offerJournalCopy(j),100)}
