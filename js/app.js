...
function setupPad(canvas,status,clearBtn,onChange){
 let drawing=false,has=false,moved=false,last={x:0,y:0},savedImage='';
 const ctx=canvas.getContext('2d');
 function configure(){ctx.setTransform(1,0,0,1,0,0);ctx.scale(window.devicePixelRatio||1,window.devicePixelRatio||1);ctx.lineWidth=2.2;ctx.lineCap='round';ctx.lineJoin='round';ctx.strokeStyle='#111'}
 function resize(){
