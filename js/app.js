...
function openJournal(id){const j=journals().find(x=>x.id===id);if(!j)return;editingJournalId=id;renderJournalEditSummary(j);for(let i=1;i<=6;i++){const c=(j.contact||[])[i-1]||'',q=(j.quality||[])[i-1]||'';$('#journalContact'+i).value=c;$('#journalContactCount'+i).textContent=c.length;$('#journalQuality'+i).value=q;$('#journalQualityCount'+i).textContent=q.length}go('journal-edit')}
