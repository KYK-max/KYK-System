...
function hydrateMasters(){['#site','#searchSite','#approvalSite','#journalSearchSite'].forEach(x=>selectOptions($(x),master.sites,x==='#site'?'選択してください':'すべて'));['#company','#searchCompany'].forEach(x=>selectOptions($(x),master.companies,x==='#company'?'選択してください':'すべて'));$$('.health-select').forEach(x=>selectOptions(x,master.health,'未選択'));$$('.qual-select').forEach(x=>selectOptions(x,master.qualifications,'未選択'));$$('.poss-select').forEach(x=>selectOptions(x,master.possibilities,'選択'));$$('.sev-select').forEach(x=>selectOptions(x,master.severities,'選択'));for(let i=1;i<=8;i++)updateQualificationDisplay(i)}
